...
CELL_target = np.load('/Volumes/microSD/COGS118a/CELL_target.npy')

print(f'CELL shape: {CELL_data.shape}')

samples, features = CELL_data.shape
        
def log(message):
    t = datetime.datetime.now()
    
    with open('CELL.log', 'a') as logf:
...
param_space = [
    ({'n_estimators': Integer(2, 1024)}, 20),
    ({'max_features': Integer(1, features-1)}, 24),
    ({'n_estimators': Integer(2, 1024), 'learning_rate': Real(1e-6, 100, prior='log-uniform')}, 20),
    ({'C': Real(1e-7, 1000, prior='log-uniform')}, 20),
    ({'gamma': Real(0.001, 2, prior='log-uniform'), 'C': Real(1e-7, 1000, prior='log-uniform')}, 12),
    ({'weights': ['uniform', 'distance']}, 16),
...
def train_val(classifier, name, partition, trial, params, n_iter, X_train, y_train):
    bscv = BayesSearchCV(classifier, params, n_iter=n_iter, cv=kf, scoring='f1', return_train_score=True, n_jobs=-1)
...
def run_trial(trial_num):
    ...
    for partition in partitions:
        ...
        X_train_val, X_test, y_train_val, y_test = train_test_split(CELL_data, CELL_target, test_size=(1-partition), random_state=seed)
        
        split = len(X_train_val)
                
        log(f'TRIAL #{trial_num+1}, TRAIN SZ: {split}, TEST SZ: {samples-split}')
